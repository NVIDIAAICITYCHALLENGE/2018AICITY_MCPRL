...
def triplet_loss(features,labels,margin):
    ...
    for i in xrange(num):
        ...
        for j in xrange(left_p,right_p):
            ...
            dist_an_all=dist_an_all.view(-1,)
            dist_an=dist_an_all.min()
            dist_ap_new=dist_ap.expand(dist_an_all.size())

            num_ap_an+=torch.sum(torch.ge(dist_ap_new.data,dist_an_all.data))  #nums of ap>=an(error) to make ap<=an
            all_triplets_num+=dist_ap_new.size()[0]
    ...
    for i in xrange(num):
        ...
        dist_an_all_end=torch.cat((dist_an_all_end,dist_an),0)
        dist_ap_end=torch.cat((dist_ap_end,dist_ap),0)
...
def select(features,labels):
    ...
    for i in xrange(num):
        ...
        if left_p==0:
            dist_an_all=distall[i][right_p+1:]
            dist_an,order_an=dist_an_all.sort()
            order_an=order_an[0].data.cpu().numpy()[0]+right_p+1
        else:
            if right_p==len(labels)-1:
                dist_an_all=distall[i][0:left_p]
                dist_an,order_an=dist_an_all.sort()
                order_an=order_an[0].data.cpu().numpy()[0]
            else:
                dist_an_all_left=distall[i][0:left_p]
                dist_an_all_right=distall[i][right_p+1:]
                dist_an_left,order_an_left=dist_an_all_left.sort()
                dist_an_right,order_an_right=dist_an_all_right.sort()

                dist_an_left=dist_an_left[0].data.cpu().numpy()[0]
                order_an_left=order_an_left[0].data.cpu().numpy()[0]


                dist_an_right=dist_an_right[0].data.cpu().numpy()[0]
                order_an_right=order_an_right[0].data.cpu().numpy()[0]+right_p+1


                if dist_an_left<=dist_an_right:
                    order_an=order_an_left
                else:
                    order_an=order_an_right

                #dist_an_all=torch.cat((distall[i][0:left_p],distall[i][right_p+1:]),0)

        dist_ap_all=distall[i][left_p:right_p+1]
        dist_ap,order_ap=dist_ap_all.sort()
        order_ap=order_ap[-1].data.cpu().numpy()[0]


        anchor.append(i)
        positive.append(left_p+order_ap)
        negative.append(order_an)
...
if __name__=="__main__":
    ...
    # labels=torch.LongTensor([1,1,5])
    # margin=torch.Tensor([1])
    # inputs,labels,margin=inputs.cuda(),labels.cuda(),margin.cuda()
    # inputs,labels,margin=Variable(inputs),Variable(labels),Variable(margin)
    # online=online_triplet_loss()
    # online.forward(inputs,labels,margin)
